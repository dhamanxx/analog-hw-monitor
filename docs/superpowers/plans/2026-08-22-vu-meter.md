...
public class AudioLevelSensorLifecycleTests
{
    ...
    public void Dispose_StopsAndDisposesTheCapture()
    {
        ...
        Assert.Equal(1, capture.StopCount + capture.DisposeCount - 1);
...
There is no unit test here, on purpose: this class exists to be the only place NAudio is visible, exactly as `SerialPortAdapter` is the only place `System.IO.Ports` is visible. It is verified by hand at the end of Task 9.
...
### Task 9: Wire it up — composition root and tray

This is the first task after which the feature can be run and seen.

**Files:**
- Modify: `AnalogHwMonitor.App/Program.cs:45-85`
- Modify: `AnalogHwMonitor.App/TrayApplicationContext.cs` (menu, timer interval, tray state)

**Interfaces:**
- Consumes: `AudioLevelSensorSource`, `WasapiLoopbackAdapter`, `ThrottledSensorSource`, `VuModeSwitch`.
- Produces: `TrayApplicationContext.SetVuMode(bool enabled)` (public, so the settings window can call it through a delegate in Task 10) and `TrayApplicationContext.IsVuMode { get; }`.
...
`ShowSettings` needs one more argument once Task 10 changes the form's constructor; leave it as it is for now and this task will not compile against the new form until then. Build order inside this task: Step 4 changes only the tray icon assignment, and the full build is checked in Task 10.
...
- [ ] **Step 5: Commit (the build is completed by Task 10)**

```bash
git add AnalogHwMonitor.App/Program.cs AnalogHwMonitor.App/TrayApplicationContext.cs
git commit -m "feat: switch the tick rate and the meters from the tray menu"
```

---

### Task 10: The settings window

**Files:**
- Modify: `AnalogHwMonitor.App/SettingsForm.cs`
- Modify: `AnalogHwMonitor.App/TrayApplicationContext.cs:61` (pass the toggle to the form)

**Interfaces:**
- Consumes: `TrayApplicationContext.SetVuMode(bool)`, `TrayApplicationContext.IsVuMode` (Task 9).
- Produces: `SettingsForm(MonitorService monitor, SerialMeterLink link, ConfigStore store, ISensorSource sensors, IAppLog log, Action<bool> setVuMode)` and `void SettingsForm.ReloadChannels(IReadOnlyList<SensorDescriptor> available)`.
...
- [ ] **Step 2: Take the toggle in the constructor and wire the checkboxes**
...
- [ ] **Step 3: Move the row building into a method the reload can share**
...
- [ ] **Step 5: Keep the volume checkbox honest on Save**
...
- [ ] **Step 6: Pass the toggle in from the tray**
...
- [ ] **Step 8: Verify by hand — this is the only test the adapter gets**
...
```bash
git add AnalogHwMonitor.App/SettingsForm.cs AnalogHwMonitor.App/TrayApplicationContext.cs
git commit -m "feat: turn VU meter mode on from the settings window"
```
...
Read the new sections against what Task 10's manual run actually did. Every claim must be one you watched happen — particularly the volume compensation behaviour, which depends on the sound card.
...
**Placeholder scan.** No "TBD", no "add error handling", no "similar to Task N", no test described without its code. Task 8's adapter has no unit test by design, and the plan says so and replaces it with the ten-point manual check in Task 10 Step 8.

**Type consistency.** Checked the names that cross task boundaries: `VuIntegrator.Add(block, offset, stride, sampleRate)` and `.Decay(TimeSpan)` are used with those exact arguments in Task 6. `AudioSensorIds.Left`/`.Right`/`.Unit`/`.FloorDbfs` are defined in Task 5 and used in Tasks 5, 6. `VuModeSwitch.Set`/`.IsUsableStash`/`.VuChannels` are defined in Task 5 and used in Tasks 4 (doc comment), 5, 9. `IAudioLoopbackCapture`'s seven members are defined in Task 6 and all seven are implemented in Task 8 (`TryStart`, `Stop`, `Format`, `DeviceId`, `DeviceName`, `CurrentDefaultDeviceId`, `VolumeDb`, `IsMuted` — eight, all present). `SettingsForm.ReloadChannels(IReadOnlyList<SensorDescriptor>)` is defined in Task 10 and called in Task 9; the plan notes that Task 9 does not build alone because of it, and Task 10 Step 7 is where the build is verified.

One ordering hazard worth restating: **Task 9 does not compile on its own.** It calls `SettingsForm.ReloadChannels` and passes a sixth constructor argument, both of which Task 10 adds. Run `dotnet build` at the end of Task 10, not at the end of Task 9.
